...
def test_two_layers_of_four_fixed_actions():
    assert list(LAYERS) == ["dancing", "petting"] and all(len(a) == 4 for a in LAYERS.values())  # the pad's order: dancing, then petting
...
def test_keys_fire_on_key_down_only_and_release_does_nothing():
    ...
    assert km.feed(ev("E", True, 12.0), 12.0) == ("head_pat", 12.0)
    assert km.feed(ev("H", True, 13.0), 13.0) == ("belly_rub", 13.0)
    assert km.feed(ev("Q", True, 14.0), 14.0) is None  # not on the pad
    assert km.lookup("F") == ("petting", "chin_scratch") and km.lookup("I") is None  # the old caring layer is gone


def test_setting_a_slot_moves_the_key_and_validates():
    km = KeyMap()
    km.set("petting", 0, "f1")
    assert km.layers["petting"] == ["F1", "F", "G", "H"]
