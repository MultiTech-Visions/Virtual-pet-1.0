...
def test_three_layers_of_four_fixed_actions():
    assert list(LAYERS) == ["dancing", "caring", "petting"] and all(len(a) == 4 for a in LAYERS.values())
...
def test_keys_fire_on_key_down_only_and_release_does_nothing():
    ...
    assert km.feed(ev("E", True, 12.0), 12.0) == ("snack", 12.0)
    assert km.feed(ev("L", True, 13.0), 13.0) == ("belly_rub", 13.0)
    assert km.feed(ev("Q", True, 14.0), 14.0) is None  # not on the pad
    assert km.lookup("F") == ("caring", "mushroom") and km.lookup("Q") is None


def test_setting_a_slot_moves_the_key_and_validates():
    km = KeyMap()
    km.set("caring", 0, "f1")
    assert km.layers["caring"] == ["F1", "F", "G", "H"]
